...
# Jumping ants
# =============================================================================
def jump(alpha, beta, net, gx, path, alg):
    ...
    try:
        ...
        t = neigh[np.random.choice(neigh, 1)[0]]
    return t
...
# Tour costruction
# =============================================================================
def walk(alpha, beta, net, gx, K, alg):
    ...
    for j in range(K-1):
        ...
        if len(neigh) > 0:
            ...
            path.append(jump(alpha, beta, net, gx, path, alg))
...
def MaxMin_AS_sim(params, alg):
    ...
    alpha, beta, rho, ph_min, ph_max, gx, net, greedy, best_theo, ants, K, trials, sim = params
    ...
    for trial in range(trials):
        antPathway = {i:walk(alpha, beta, net, gx, K, alg) for i in range(ants)}
    ...
    with PdfPages('Convergence_%i_%i_%i.pdf'%(K, trials, sim)) as pdf:
        fig, ax = plt.subplots()
        scores = [[antRes[i][j][1] for j in range(ants)] for i in range(trials)]
        best = [np.max(s) for s in scores]
        avg = [np.mean(s) for s in scores]
        ...
        res_df.to_csv('antpaths_%i_%i_%i.txt'%(K, trials, sim), sep='\t')
        theo_df = pd.DataFrame(columns=['path', 'score'])
        for i in range(len(best_theo)):
            theo_df.loc[i,:] = best_theo[i]
        theo_df.to_csv('best_%i_%i_%i.txt'%(K, trials, sim), sep='\t')
        pd.DataFrame(W).to_csv('netEdges_%i_%i_%i.txt'%(K, trials, sim), sep='\t')
...
def MaxMin_AS(params, alg):
    ...
    alpha, beta, rho, ph_min, ph_max, gx, net, ants, K, trials, sim = params
